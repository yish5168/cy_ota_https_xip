...
typedef void (*cy_fn_boot_jump_ptr_t)(void);
...
static void do_boot(struct boot_rsp *rsp)
{
    ...
    BOOT_LOG_INF("Starting User Application. Please wait...app_addr:0x%x",app_addr);
    Cy_SysLib_Delay(CM4_BOOT_DELAY_MS);
    //Cy_SysEnableCM4(app_addr);
    cy_boot_appId = 1;
    cy_boot_appAddr = app_addr;
    CySoftwareReset();
...
int main(void)
{
    ...
    printf("\n\n*****************************************************\n");
    printf("MCUboot Bootloader Started.\n");
    printf("Bootloader CM0 Build Date: %s - Time:%s\n", __DATE__, __TIME__);
    printf("*****************************************************\n\n");
...
{
    /* Set cy_boot_appId to ZERO under a non-software reset. This means
     * that the bootloader application is scheduled - the initial clean state.
    ...
    {
        if ((cy_boot_appId == 1u) && (cy_boot_appAddr >= (CY_FLASH_BASE + CY_BOOT_BOOTLOADER_SIZE)))
        {
            uint32_t startAddress = cy_boot_appAddr;
            ...
            ((cy_fn_boot_jump_ptr_t) resetHandler) ();
